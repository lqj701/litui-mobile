...
class ImInit {
      ...
      this.getImAccount().then(() => {
        this.login().then(() => {
          this.getContactList().then(({ contacts, customerList }) => {
            // 查找所有联系人的账号，获取的未读数
            const accountIds = customerList ? customerList.map(account => account.accountId): [];
            this.store
              .dispatch(
                fetchUnreadNum({
                  toAccountId: this.imAccount.accountId,
  ...
  syncMsgs(contacts, customerList) {
    ...
    webim.syncMsgs(() => {
      ...
      contacts.forEach(o => {
        const customer = find(customerList, { accountId: o.toAccount });
        if (!customer) return;
        ...
        list.push(
          Object.assign(o, {
            toAccountAvatar: customer.faceUrl,
  ...
  getContactList() {
    return imsdk.getRecentContactList().then(contacts => {
      ...
      return this.store
        ...
        .then(act => {
          return { contacts, customerList: act.payload };
        });
    });
  }
  ...
  getImAccount() {
    return this.store
      ...
      .then(act => {
        this.imAccount = act.payload.imAccount;
      });
  }

  getWxUser() {
    return this.store.dispatch(getUserInfo()).then(act => {
      this.wxUser = act.payload.wxUser;
    });
  }
  ...
  loginInfo() {
    return {
      identifier: this.imAccount.accountId,
      identifierNick: this.imAccount.nikeName,
      userSig: this.imAccount.signPassword
    };
  }
